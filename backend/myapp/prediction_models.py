...
import numpy as np
import os
from myapp.utils import convert_is_placed_to_zero_ifnot_placed, load_pickle_models, transform_prediction
from myapp.ml_deploy import predict_isplaced_api, predict_salary_api
...
def get_predicted_data(dataset):
    try:
        print("[INFO] Original input dataset:")
        print(dataset)

        # Transform features
        placed_prediction_feature_data = transform_prediction(dataset)
        print("[INFO] Transformed data for placement prediction:")
        print(placed_prediction_feature_data)

        salary_prediction_feature_data = transform_prediction(dataset)
        print("[INFO] Transformed data for salary prediction:")
        print(salary_prediction_feature_data)

        # Get placement probability from API
        placement_probability = predict_isplaced_api(placed_prediction_feature_data)
        print("[INFO] Placement probability response:", placement_probability)

        if isinstance(placement_probability, float):
            placement_probability = [placement_probability]
        elif isinstance(placement_probability, dict) and 'probability' in placement_probability:
            placement_probability = [placement_probability['probability']]

        isplaced_predictions = [1 if prob > 0.6 else 0 for prob in placement_probability]
        print("[INFO] is_placed predictions:", isplaced_predictions)

        # Predict salary
        salary_predictions = predict_salary_api(salary_prediction_feature_data)
        print("[INFO] Salary predictions:", salary_predictions)

        # Fix salary if not placed
        salary_predictions = convert_is_placed_to_zero_ifnot_placed(
            isplaced_predictions, salary_predictions
        )
        print("[INFO] Final adjusted salary predictions:", salary_predictions)

        # Add results to dataset
        dataset = dataset.reset_index(drop=True)
        dataset['placement_probability'] = placement_probability
        dataset['salary_as_fresher'] = salary_predictions
        dataset['is_placed'] = isplaced_predictions

        print("[✅ SUCCESS] Final predicted dataset:")
        print(dataset)

        return dataset

    except KeyError as e:
        print(f"[KeyError] Missing expected key in dataset: {e}")
        return pd.DataFrame([{'error': f'Missing expected key in dataset: {e}'}])

    except IndexError as e:
        print(f"[IndexError] Index issue while processing data: {e}")
        return pd.DataFrame([{'error': f'Index issue while processing data: {e}'}])

    except Exception as e:
        print(f"[Exception] An error occurred in get_predicted_data: {str(e)}")
