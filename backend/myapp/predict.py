...
from myapp.utils import save_df_to_temp
...
def predict_students_placement(student_dict):
    try:
        dataframe = pd.DataFrame(student_dict)
        print("[INFO] Input DataFrame:", dataframe)

        df_predicted = get_predicted_data(dataframe)
        print("[INFO] Predicted DataFrame:", df_predicted)

        is_placed = int(df_predicted.loc[0, 'is_placed'])
        predicted_salary = float(df_predicted.loc[0, 'salary_as_fresher'])
        placement_probability = float(df_predicted.loc[0, 'placement_probability'])

        result = {
            'placement_predictions': is_placed,
            'placement_probability': placement_probability,
            'salary_prediction': predicted_salary
        }

        print("[INFO] Final prediction result:", result)
        return result

    except KeyError as e:
        print(f"[KeyError] Missing expected column or key: {e}")
        return {'error': f'Missing expected column or key: {e}'}

    except IndexError as e:
        print(f"[IndexError] DataFrame might be empty or malformed: {e}")
        return {'error': f'DataFrame might be empty or malformed: {e}'}
...
def predict_college_stats(excel_file):
    ...
    original_df = load_data(excel_file=excel_file)

    predicted_rows = []  # Store each student's prediction

    for _, row in original_df.iterrows():
        try:
            student_df = pd.DataFrame([row])  # Create single-row DataFrame
            prediction = get_predicted_data(student_df)

            # Ensure prediction is a valid DataFrame
            if isinstance(prediction, pd.DataFrame):
                predicted_rows.append(prediction)
            else:
                predicted_rows.append(pd.DataFrame([{'error': 'Invalid prediction format'}]))
        except Exception as e:
            print(f"⚠️ Error predicting for row: {e}")
            predicted_rows.append(pd.DataFrame([{'error': str(e)}]))

    # Combine all results
    df = pd.concat(predicted_rows, ignore_index=True)

    if 'error' not in df.columns:
        avg_salary = df['salary_as_fresher'].mean()
        avg_probability = df['placement_probability'].mean()
        is_placed_count = df['is_placed'].sum()
        total_students = len(df)

        print(f"📊 Avg Salary: {avg_salary}, Avg Placement Probability: {avg_probability}, Placed: {is_placed_count}/{total_students}")
    else:
        print("⚠️ Prediction failed:", df)
